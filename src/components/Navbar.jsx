...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 overflow-visible border-b border-white/10 bg-footer-blue text-white shadow-[0_3px_10px_rgba(6,27,97,0.16)] transition-shadow ${scrolled ? "shadow-[0_14px_35px_rgba(6,27,97,0.3)]" : ""}`}
    >
      <div className="relative mx-auto flex h-[82px] w-[calc(100%-48px)] max-w-[1280px] items-center justify-between max-sm:w-full max-sm:px-3">
        ...
        >
          <span className="flex flex-col text-[1.08rem] font-bold leading-none tracking-[-.04em]">
            Service
          </span>
        </Link>
        <nav
